...
if($action == 'create') {
	...
	if($method == 'GET') {
		...
		// 和传承公众号链接开始
		$inh_arr = array('title' => $subject, 'user_id' => $uid, 'add_time' => $time, 'is_open' => '1', 'last_time' => $time);
		$inh_id = ci_cc_create($inh_arr,'ci_cc_inherit');
		if($inh_id){
			$thread['tid'] = $inh_id;
		}
		// 插入传记权限表
		$inh_power_arr = array('inh_id' => $inh_id, 'user_id' => $uid, 'power_form' => '创建者', 'add_time' => $time);
		ci_cc_create($inh_power_arr, 'ci_cc_inherit_power');
		// 和传承公众号链接结束
