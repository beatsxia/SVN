...
// 修改传承内容表
function ci_cc_update($table,$cond, $update)
{
	$db = $_SERVER['db'];
	$d = $db;
	if(!$d) return FALSE;
	...
	db_exec("UPDATE $table SET $sqladd $condadd", $d);
}

// 插入传承内容表
function ci_cc_insert($table, $arr)
{
	$db = $_SERVER['db'];
	$d = $db;
	if(!$d) return FALSE;
	...
	if(!$sqladd) return FALSE;
	return db_exec("INSERT INTO $table $sqladd", $d);
}
