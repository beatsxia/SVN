...
function attach_assoc_post($pid) {
	...
	// 将图片存储到七牛云开始
	require_once APP_PATH.'plugin/php-sdk-master/autoload.php';
	require_once APP_PATH.'plugin/php-sdk-master/qiniu_upload.php';
	$qiniu_conf = require_once APP_PATH.'conf/qiniu.conf.php';

	// 需要填写你的 Access Key 和 Secret Key
	$accessKey = $qiniu_conf['accessKey'];
	$secretKey = $qiniu_conf['secretKey'];
	...
	foreach($tmp_files as $file) {
		...
		// 图片上传到七牛空间
		$ret = qiniu_putfile($accessKey,$secretKey,$bucket,$destfile,$desturl);
		if($ret){
			$desturl = $qiniu_conf['url'].$url.'/'.$filename;
		}
		// 将图片存储到七牛云结束
